...
const composeMessage = (evaluation) => {
    ...
    evaluation.who.forEach((entry) => {
        ...
        message += `**${entry.ingame_name}** - Hat insgesamt schon ${formattedNumber(entry.donation_total)} Gold gespendet\n`;
    });
    ...
    evaluation.who.forEach((entry) => {
        ...
        if (entry.expiration_date >= Date.now()) // not expired
            message += `${entry.ingame_name} hat ${formattedNumber(entry.donation_amount)} / ${formattedNumber(entry.donation_amount_weekly)} Gold gespendet und ist bis zum ${dateFormat(entry.expiration_date, 'dd.mm.yyyy')} befreit. :white_check_mark:\n`;
        else // expired
            message += `${entry.ingame_name} hat ${formattedNumber(entry.donation_amount)} / ${formattedNumber(entry.donation_amount_weekly)} Gold gespendet und muss seit dem ${dateFormat(entry.expiration_date, 'dd.mm.yyyy')} aufholen. :x:\n`;
    });
